...
def main():
    ...
    # ۱.۱. همگام‌سازیِ خودکارِ رمزِ همه‌ی حساب‌ها با آیدیِ عددیِ تلگرامشون —
    # هم برای کاربرانِ قدیمی، هم برای هر کاربرِ تازه‌ای که از قبلِ این دیپلوی
    # وارد تلگرامش شده. از این به بعد هر بار برنامه بالا بیاد این چک تکرار
    # می‌شه، پس نیازی به اجرای دستیِ کوئری SQL نیست.
    try:
        db.sync_all_passwords_with_telegram_id()
    except Exception as e:
        print(f"⚠️ خطا در همگام‌سازیِ رمزها با آیدیِ عددیِ تلگرام: {e}")
